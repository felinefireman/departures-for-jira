...
One Cloudflare Worker, no runtime dependencies (wrangler is the only devDependency), no
database, nothing to build. Every metric comes from Jira's `search/approximate-count`
endpoint, which returns a single number and nothing else: no issue titles, no assignees, no
descriptions, never requested, so there's no per-person data to leak. I recommend using
...
# or
npm run demo    # the fuller demo board, the one in the screenshots
```

Open <http://localhost:8787>. With no `.dev.vars`, the Worker has no Jira token, so it renders
on synthetic data: no Jira account, no Cloudflare account, no API token needed.

To test locally against your real Jira data instead, create a `.dev.vars` file in the repo
root (it's gitignored, never committed) with `JIRA_HOST=`, `JIRA_EMAIL=`, and `JIRA_TOKEN=` on
their own lines.

## How it works

Cron writes, page loads read. A page load never queries Jira, so a wall of TVs costs the same
as one.
...
The interesting constraint is Cloudflare's Free plan cap of 50 subrequests per invocation, and
...
A config too big for one invocation costs update latency instead of failing, and
`partial: true` says so while the first sweep is still filling in. The config builder shows
...
2. Create a Jira API token at <https://id.atlassian.com/manage-profile/security/api-tokens> and
   ...
   JIRA_HOST=your-org.atlassian.net JIRA_EMAIL=you@example.com JIRA_TOKEN=<paste your token> \
     npm run discover
   ```
   Then open [`tools/config-builder.html`](tools/config-builder.html) in a browser (a static
   form, no server, no Jira connection) and type in what `discover` printed: team names,
...
5. Open `wrangler.jsonc` in a text editor and replace the placeholders: `JIRA_HOST` is your
   ...
   your Jira account, the same one behind the token from step 2. Neither is a secret, so a
   plain text edit is fine.
