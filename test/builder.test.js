...
// config-builder.html is standalone — it duplicates the query arithmetic and slug rule.
// Pin the copies to the originals so they can't drift.
...
describe("config builder", () => {
  ...
  // Its whole job is emitting a file that runs — round-trip a config through serialise()
  // and check it parses back identical.
  it("emits config text that parses back identically", async () => {
    // Everything before event wiring is DOM-free function defs.
  ...
  // "Valid" = validateConfig won't reject it at startup. Pin problems() to the same
  // checks so they can't drift apart.
  describe("problems() mirrors validateConfig", () => {
    // problems() touches module-level KEY_RE/slugify(), so it needs defs in scope —
    // lift()'s bare body won't do.
    const defs = SCRIPT.slice(0, SCRIPT.indexOf('["bname"'));
