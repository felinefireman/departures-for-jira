...
// Reads the source, not the rendered output: rendered HTML can never contain an
// interpolation, so checking it there proves nothing. See CONTRIBUTING.md for why the
// script region must stay ES5.
...
describe("client script region", () => {
  ...
  // Everything the client needs rides inside CFG; a second injection means someone is
  // hand-building JS again.
  ...
  // A scene renderer that hardcodes its target overwrites whichever scene lands on that id;
  // this is the bug that made a second spotlight overwrite the first.
  ...
  // A spotlight named "A" slugifies to "a", which is also scene 0's generated id. One
  // shared map let the slug win and sent ?scene=a to the wrong scene.
...
describe("renderDashboard", () => {
  ...
  it("includes the DEMO DATA chip markup", () => {
    assert.ok(html.includes("DEMO DATA"), "no demo chip in template");
  });

  // JSON.stringify escapes quotes but not "<", so a config string could close the script
  // element and inject markup after it.
